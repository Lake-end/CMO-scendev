-- ========================================================
-- MASTER TICKER: EVENT EXECUTION ENGINE V2.6 (Debug & Fixes)
-- ========================================================
...
if convoyGroup ~= nil and convoyGroup.group ~= nil then
    -- Haetaan ryhmän johtaja suoraan GUIDilla, kuten esimerkissäsi
    ...
    if leadShip ~= nil then
        ...
        leadGUID = leadShip.guid -- Käytetään tätä myöhemmin miinakentän ankkurointiin
...
for _, id in ipairs(eventsToExecute) do
    ...
    elseif id == 5 then
        ...
        if roll == 1 then
            LogEvent("EVENT 5: Spawnaan Russian Sub koordinaatteihin: Lat " .. (cLat+latOff) .. ", Lon " .. (cLon+lonOff))
            ScenEdit_AddUnit({type="Submarine", name="Unknown Sub", dbid=748, side=sideRussia, latitude=cLat+latOff, longitude=cLon+lonOff, depth=-150})
        elseif roll == 3 then
            LogEvent("EVENT 5: Spawnaan NATO Hunter Sub koordinaatteihin: Lat " .. (cLat+latOff) .. ", Lon " .. (cLon+lonOff))
            ScenEdit_AddUnit({type="Submarine", name="SSN 751 San Juan", dbid=837, side=sideUnknown, latitude=cLat+latOff, longitude=cLon+lonOff, depth=-200})
        else
            LogEvent("EVENT 5: False Alarm, ei spawnata mitään.")
        end
    ...
    elseif id == 6 then
        ...
        ScenEdit_SpecialMessage(sideNATO, "FLASH INTEL: Alliance acoustic intelligence has detected distinct acoustic signatures matching mine-laying operations in your sector. Divert course 90 degrees or proceed with extreme ASW/MCM caution.") 
        ...
        if leadGUID ~= nil then
            ...
            -- 2. Vaihdetaan RP:n side Venäjälle (Jotta miinakenttä voi kuulua sille)
            for _, rpName in ipairs(rpNames) do
                ScenEdit_SetReferencePoint({side=sideRussia, name=rpName, clear=true})
            end

            LogEvent("EVENT 7: Miinakentän RP:t luotu ja siirretty Venäjän sidelle.")
    ...
    elseif id == 8 then
        ...
        if u ~= nil then
            ...
            -- FIX: Koska "u" luotiin juuri nyt, RelativeTo kaatuu. Luomme RP:t kiinteisiin koordinaatteihin laivan ympärille.
