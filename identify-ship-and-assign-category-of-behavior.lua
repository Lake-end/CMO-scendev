-- ========================================================
-- VISUAL INSPECTION & RECLASSIFICATION ENGINE V1.35
-- ========================================================
...
local sides = {
    {
        sideName = "side1", 
        chance = 0.20, -- should be 0.10
        namePrefix = "Suspected shadow fleet", 
        mission = "Shadow Fleet Mission",
        intelMsg = "Vessel displays signs of ownership obfuscation, dark AIS history, or non-standard cargo transfers. Suspected sanctions-evading shadow tanker. Monitor closely for anomalous maneuvers."
    },
    {
        sideName = "side2", 
        chance = 0.20, 
    ...
    {
        sideName = "side5", 
        chance = 0.20,  -- should be 0.05
        namePrefix = "Potential immediate threat", 
        mission = "Threat Mission",
        intelMsg = "CRITICAL: Armed personnel or specialized military-grade equipment observed on deck. Vessel is suspected of acting in direct violation of international law or regulations, suspected threat to safety of friendly infrastructure. Prepare to board immediately and exercise extreme caution. Use closest air or seaborne special forces units to intercept."
    }
}
...
if targetUnit ~= nil then
    ...
    -- DYNAAMINEN RNG SIEMENNYS: Yhdistetään kellonaika ja GUIDin ascii-arvot.
    -- Tämä estää identtiset satunnaisluvut, jos kaksi eventtiä laukeaa samana sekuntina!
    local guidHash = 0
    for i = 1, #guid do
        guidHash = guidHash + string.byte(guid, i)
    end
    ...
    local sideChangeSuccess = ScenEdit_SetUnitSide({guid = guid, side = targetUnit.side, newside = selectedCategory.sideName})

    -- Vaihe B: Jos puolen vaihto onnistui, päivitetään nimi ja kytketään tehtävään
    if tonumber(sideChangeSuccess) == 1 or sideChangeSuccess == true then
        -- Vaihdetaan uusi nimi samalla toimivalla GUIDilla
        ScenEdit_SetUnit({guid = guid, newname = newShipName})
